...
function speakGameInstructions(event) {
  event.preventDefault();
  speakTextWithAzure(
    "동화책에서 만든 문장을 우리 읽어보자 듣기 버튼을 눌러서 각 문장을 들어봐 ! 마이크 버튼을 눌러서 각 문장을 읽어줘! 문장을 다 읽어봐야지만 다음으로 넘어갈 수 있어 !"
  );
}
    ...
    mediaRecorder.onstop = async () => {
      ...
      if ("webkitSpeechRecognition" in window) {
        ...
        recognition.onresult = async (event) => {
          ...
          console.log(`🎤 사용자가 말한 내용: ${userText}`);

          // 🎯 정확도 평가
          const accuracy = calculateAccuracy(expectedText, userText);
          document.getElementById(
            `accuracy-${sentenceIndex}`
          ).innerText = `정확도: ${accuracy}%`;

          // 🎯 정확도를 서버로 전송
          await fetch("https://52.141.27.150/submit_evaluation", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({
              game_name: "말하기", // "말하기" 카테고리로 저장
              success: accuracy / 100, // 0~1 범위로 변환
            }),
          });
        };
